...
<template>
    <div>
        ...
        <section id="section1" class="bg-[#F8F4EC] w-full justify-center pt-6 md:pt-0">
            <div class="w-full">
                <div class="flex flex-wrap gap-4 w-[90%] mx-auto">
                    <div class="relative group" data-aos="zoom-in" v-for="(cluster) in clusters.data.data"
                        :key="cluster.id">
                        <img :src="`${config.public.dirUrl}/${cluster.image}`" alt="Photo 1"
                        ...
                        <div
                            ...
                            <button
                                @click="showClusterGalery({ images: [cluster.image, cluster.image2, ...cluster.facade.map(facade => facade.image)], description: cluster.name })"
                                class="mt-1 text-white mb-5 ml-5 group-hover:text-[#38544c] transition-all duration-300 text-[10px] font-bold">VIEW
                                GALLERY</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-8 pb-10 text-center">
                    <button class="flex items-center pt-5 mx-auto btn-visit-black" data-aos="fade-up"
                        data-aos-delay="300">
